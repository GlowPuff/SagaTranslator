...
{
	public class MissionEvent : INotifyPropertyChanged, IFindReplace
	{
		...
		public int FindReplace( string needle, string replace )
		{
			...
			if ( !string.IsNullOrEmpty( eventText ) )
			{
				...
				count += regex.Matches( eventText ).Count;
				foreach ( var match in regex.Matches( eventText ) )
				{
					eventText = eventText.Replace( match.ToString(), replace );
				}
			}
			return count;
		}
