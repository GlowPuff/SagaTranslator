...
{
	public class CustomEnemyDeployment : EventAction, IFindReplace
	{
		...
		{
			int count = 0;
			if ( !string.IsNullOrEmpty( surges ) )
			{
				var regex = new Regex( needle );
				count += regex.Matches( surges ).Count;
				foreach ( var match in regex.Matches( surges ) )
				{
					surges = surges.Replace( match.ToString(), replace );
				}
			}
			...
			{
				var regex = new Regex( needle );
				count += regex.Matches( keywords ).Count;
				foreach ( var match in regex.Matches( keywords ) )
				{
					keywords = keywords.Replace( match.ToString(), replace );
				}
			}
			...
			{
				var regex = new Regex( needle );
				count += regex.Matches( bonuses ).Count;
				foreach ( var match in regex.Matches( bonuses ) )
				{
					bonuses = bonuses.Replace( match.ToString(), replace );
				}
			}
			...
			{
				var regex = new Regex( needle );
				count += regex.Matches( abilities ).Count;
				foreach ( var match in regex.Matches( abilities ) )
				{
					abilities = abilities.Replace( match.ToString(), replace );
				}
			}

			if ( !string.IsNullOrEmpty( repositionInstructions ) )
			{
				var regex = new Regex( needle );
				count += regex.Matches( repositionInstructions ).Count;
				foreach ( var match in regex.Matches( repositionInstructions ) )
				{
					repositionInstructions = repositionInstructions.Replace( match.ToString(), replace );
				}
			}
